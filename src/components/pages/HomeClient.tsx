...
export function HomeClient() {
  ...
  // Hydration-safe: server and client initial render both use DEFAULT_MARKET;
  // user's stored preference is applied in the mount-only useEffect below.
  ...
  // Hydration-safe: server always returns DEFAULT_USER_PREFERENCES;
  // client-side preferences are applied after mount.
  const [preferences, setPreferences] = useState(DEFAULT_USER_PREFERENCES)

  // backward-compat alias used by SkeletonList in movers section
  const loading = loadingMovers

  // Apply stored user preferences after hydration
  useEffect(() => {
  ...
  // Mount-only: fetch movers (market-independent)
  useEffect(() => {
  ...
  // Mount-only: fetch livestock prices (egg + pork)
  useEffect(() => {
    fetchLivestock()
      .then(setLivestock)
  ...
  // Mount-only: fetch seasonal guide from MOA (top-3 by volume this month);
  // falls back to static SEASONAL_GUIDE on error so the card is never empty.
  useEffect(() => {
    fetchSeasonal()
  ...
  // Re-fetch overview + trend whenever market or reloadKey changes
  useEffect(() => {
    ...
    async function loadOverviewAndTrend() {
      ...
      // Fetch overview, trend, rest-day and weather-risk in one concurrent batch.
  ...
  // Pre-compute SVG polyline points for the hero card sparkline
  ...
  // Show error card only when both overview AND movers have failed
    ...
    <div className="px-section-margin py-6 space-y-section-margin">
      ...
      <section>
        ...
        {!summaryDismissed && preferences.dailySummary && overview && !loadingOverview && (
          <div className="bg-primary/8 border border-primary/20 rounded-2xl px-4 py-3 flex items-center justify-between text-body-sm text-on-surface mb-3">
        ...
        {loadingOverview ? (
          ...
          <Link
            ...
            className="block home-hero-card rounded-3xl overflow-hidden"
          >
            {/* Price + volume row */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredMovers.length > 0 ? filteredMovers.map((item, i) => (
              <Link
                ...
                className="mover-entry glass-card rounded-2xl flex items-center justify-between p-3.5 hover:bg-white/60 transition-colors touch-target block"
              ...
              <GlassCard className="p-container-padding md:col-span-2 lg:col-span-3 text-center">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {loadingLivestock ? (
            ...
            <>
              <GlassCard className="p-container-padding">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <GlassCard className="p-container-padding flex flex-col justify-between min-h-[160px]">
          <div>
            ...
            {loadingSeasonal ? (
              ...
              seasonalGuide.map((item) => (
                <Link
                  ...
                  className="shrink-0 w-48 rounded-3xl glass-card p-4 hover:bg-white transition-all shadow-glass-sm hover:shadow-glass flex flex-col snap-start border border-white/40 group"
                >
